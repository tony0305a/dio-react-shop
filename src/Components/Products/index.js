...
export const Products = () => {
  ...
  const { imageUrl, apiUrl, session, attCart, logedInState } = useStore();
  ...
  const addCart = async (userid, nome, img, quantidade, preco) => {
    ...
    fetch(`${apiUrl}/cart/add`, options).catch((err) => console.error(err));
  };
    ...
    <S.Wrapper>
      {produtos.map((item, index) => (
        <S.Item key={index}>
          ...
          <S.ItemImage src={`${imageUrl}/${item.img}`} />
          <S.ItemData>
            <S.ItemName style={{ fontColor: "lightgray" }}>
              {item.name}
            </S.ItemName>
